            ...
            for station in stations:
                ...
                if is_valid_toilet_json(position_json.strip()):
                    ...
                    for i, t in enumerate(toilets):
                        ...
                        # 英文描述从整体英文字段中按顺序分割（可优化）
                        description_en = position_en_text.strip() if isinstance(position_en_text, str) else ""

                        if description:
                            cleaned_description = description.strip()

                            # 查重：以 description + line 为准
                            cursor.execute("""
                                SELECT id FROM station_toilets 
                                WHERE description = %s AND line = %s LIMIT 1;
                            """, (cleaned_description, lineno))
                            ...
                            if not result:
                                ...
                                print(f"⚠️ 已存在: {cleaned_description} (line {lineno})，跳过。")
                else:
